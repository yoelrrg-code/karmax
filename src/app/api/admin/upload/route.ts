...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Sanitización activa para archivos SVG contra XSS almacenado y XXE
    if (file.type === "image/svg+xml") {
      let svgText = buffer.toString("utf-8");
      // Neutralizar DOCTYPE, entidades XML, scripts, foreignObject y handlers inline
      svgText = svgText
        .replace(/<\?xml[\s\S]*?\?>/gi, "")
      ...
      if (
        /<script/i.test(svgText) ||
        /javascript:/i.test(svgText) ||
        /data:text\/html/i.test(svgText) ||
        /<iframe/i.test(svgText)
      ) {
        return NextResponse.json(
          { error: "El archivo SVG contiene elementos no permitidos por seguridad." },
          { status: 400 }
        );
      }
    ...
    const targetDir = path.resolve(baseImagesDir, targetSubfolder);

    // Guard contra directory traversal
    if (!targetDir.startsWith(baseImagesDir)) {
      return NextResponse.json({ error: "Directorio de destino inválido" }, { status: 400 });
    }
